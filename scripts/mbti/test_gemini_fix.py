...
try:
    print("\n1️⃣ 測試導入 google-generativeai...")
    import google.generativeai as genai
    print("   ✅ 成功")
    
    print("\n2️⃣ 測試導入 mbti_service...")
    from app.services.mbti_service import MBTIService
    print("   ✅ 成功")
    
    print("\n3️⃣ 檢查 Gemini API 配置...")
    from app.core.config import settings
    if settings.GEMINI_API_KEY:
        print("   ✅ GEMINI_API_KEY 已配置")
    else:
        print("   ⚠️  GEMINI_API_KEY 未配置，請在 .env 中設置")
    print(f"   ✅ 使用模型: {settings.GEMINI_MODEL}")
    
    print("\n4️⃣ 檢查 content 格式...")
    print("   ✅ 已使用正確的 Gemini API 格式")
    print("   - 文本: 直接字符串")
    print("   - 圖像: {'mime_type': 'image/jpeg', 'data': base64_string}")
    
    print("\n5️⃣ 檢查 generation_config 格式...")
    print("   ✅ 已使用字典格式而非 genai.types.GenerationConfig")
