...
## 🔧 重要更新：重新整理功能修復 (2025-12-27)

### ✅ 已修正問題
1. **Webhook 部門匹配失敗** - 移除查詢時不存在的 `is_active` 欄位過濾
2. **簽名驗證增強** - 加入詳細除錯日誌、正確處理字節編碼、捕捉異常
3. **部門初始化日誌** - 顯示完整的部門載入資訊和統計
...
# 3. 確認初始化日誌
# 應該看到: [初始化] 完成，共 12 個部門可用
```

### 🧪 測試重新整理功能
1. 在 LINE Bot 中點擊任務卡下方的「重新整理」按鈕
2. 應該收到任務列表的 Carousel 輪播卡片
3. 查看服務器日誌確認簽名驗證成功:
   ```
   [Signature] ✅ 部門 HK 簽名驗證成功
   [Webhook] [OK] 簽名匹配: HK (房務部)
   ```

### 📝 修正的檔案
- [`app/routes/linebot_routes.py`](app/routes/linebot_routes.py#L46-L58) - Webhook 部門查詢
- [`app/services/linebot_service.py`](app/services/linebot_service.py#L387-L409) - 初始化和簽名驗證
